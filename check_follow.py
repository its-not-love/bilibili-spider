...
oid=147412275 #视频id
up_id=[4162287,927587] #up列表，这里是c菌和木鱼
...
def get_res():
    ...
    if result==[]:
        ...
        for r in result:
            get_detail(r)
        print("共匹配到"+str(len(result))+"位用户")
def get_detail(mid):
    url='https://api.bilibili.com/x/space/acc/info?mid={}&jsonp=jsonp'.format(mid)
    zhuye='https://space.bilibili.com/'+str(mid)
    jsondata=requests.get(url).json()['data']
    name=jsondata['name']
    level = jsondata['level']
    sign = jsondata['sign']
    sex = jsondata['sex']
    print("id:{} 昵称:{} 等级:{} 个签:{} 性别:{} 主页:{}".format(mid,name,level,sign,sex,zhuye))
#以下是解码部分
def create_table():
    for i in range(256):
        crcreg = i
